...
class ResizeImage(object):
    ...
    def __call__(self,sample):
        if self.train:
            left_image = sample['imL']
            right_image = sample['imR']
            new_right_image = self.transform(right_image)
            new_left_image = self.transform(left_image)
            sample['imL'] = new_left_image
            sample['imR'] = new_right_image
        else:
    ...
    def __call__(self, sample):
        ...
        if self.do_augmentation:
            if k > 0.5:
                ...
                dispflip = self.transform(disp)
                sample['imL'] = fliped_left
                sample['imR'] = fliped_right
                sample['dispL']=dispflip
    ...
    def __call__(self, sample):
        ...
        if self.do_augmentation:
            if p > 0.5:
                ...
                sample['imL'] = left_image_aug
                sample['imR'] = right_image_aug
